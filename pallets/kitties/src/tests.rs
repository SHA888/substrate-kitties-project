...
fn can_create() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(kittyModule::kitties(&100, 0), Some(kitty.clone()));
...
fn can_breed() {
    new_test_ext().execute_with(|| {
        ...
        let kitty = Kitty([187, 250, 235, 188, 211, 247, 237, 253, 187, 239, 191, 185, 239, 171, 211, 122]);

        assert_eq!(KittiesModule::kitties(&100, 2), Some(kitty.clone()));
        assert_eq!(Nft::tokens(KittiesModule::class_id(), 2).unwrap().owner, 100);

        System::assert_last_event(Event::KittiesModule(crate::Event::<Test>::KittyBreed(100u64, 2u32, kitty)));
    });
}
...
fn can_buy() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(KittiesModule::buy(Origin::signed(200), 100, 0, 500), Error::<T>::PriceTooLow);

        assert_noop!(KittiesModule::buy(Origin::signed(200), 100, 0, 600), pallet_balances::Error::<T>::InsufficientBalance);
